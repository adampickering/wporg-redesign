...
| `InnovatorSpotlight`   | https://tailark.com/dusk/testimonials (testimonials-5 — 3-column wall-of-love card grid) | Testimonials-5 uses `lg:grid-cols-3` with Cards inside, quote + author per card. Repurposed: avatar removed; quote gets `font-serif` for editorial warmth (not italic, per spec); author footer has name + plugin in mono eyebrow. Light bg-background surface. Shadow-card resting / shadow-card-hover on hover (shadow growth, no translateY). |
| `BuildPluginCta`       | https://tailark.com/dusk/call-to-action (call-to-action-2 — centered content in a bordered card wrapper) | Variant 2 has a `rounded-3xl border` inner container matching the spec's bg-surface-cream + hairline border + rounded-card requirement. Rethemed: cream inner surface, dark pill CTA button (bg-foreground, rounded-pill, shadow-button, active:scale-[0.97]), eyebrow + display headline + light body. No icons, no "use client". |
